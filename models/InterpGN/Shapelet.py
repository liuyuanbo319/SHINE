...
class ShapeletDistanceFunc(Function):
    ...
    def forward(ctx, x, s):
        ...
        output = torch.cat([(s - x[:, :, i:i + s.shape[-1]].unsqueeze(1)).pow(2).mean(-1).unsqueeze(1) for i in
                            range(x.shape[-1] - s.shape[-1] + 1)], dim=1)
        return output
...
class Shapelet(nn.Module):
    def __init__(self, dim_data, shapelet_len, num_shapelet=10, stride=1, eps=1., distance_func='euclidean',
...
class ShapeBottleneckModel(nn.Module):
    def __init__(
        ...
        self.num_channel = configs.in_channels
        ...
        for i, l in enumerate(shapelet_len):
            sl = max(3, np.ceil(l * configs.ts_len).astype(int))
            self.shapelets.append(
                Shapelet(
                    dim_data=self.num_channel,
                    shapelet_len=sl,
                    num_shapelet=num_shapelet[i],
                    stride=1 if configs.ts_len < 3000 else max(1, int(np.log2(sl)))
                )
            )
        ...
        self.lambda_reg = 0.1 #configs.lambda_reg  # L1 regularization on classifier weights
        self.lambda_div = 0.1 #configs.lambda_div  # Shapelet diversity loss
    ...
    def forward(self, x, *args, **kwargs):
        ...
        #     out = self.output_layer(self.dropout(shapelet_probs)) + self.output_bilinear(self.dropout(shapelet_probs),
        #                                                                                  self.dropout(shapelet_probs))
