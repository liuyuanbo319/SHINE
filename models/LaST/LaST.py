...
from models.LaST.LaST_utils import CriticFunc, NonLinear, log_Normal_diag, log_Normal_standard, NeuralFourierLayer, \
    period_sim, trend_sim
...
class VarUnit(nn.Module):
    ...
    def forward(self, x, return_para=True):
        ...
        # mean = torch.nan_to_num(mean,1e-8)
        # var = torch.nan_to_num(var,0.01)
...
class LaSTBlock(nn.Module):
    ...
    def forward(self, x_his):
        xs_rec, elbo_s, mlbo_s,qz_s = self.SNet(x_his)
        xt_rec, elbo_t, mlbo_t,qz_t= self.TNet(x_his)
        ...
        return elbo, mlbo, mubo,xs_rec,xt_rec,qz_s,qz_t
...
class SNet(nn.Module):
    ...
    def forward(self, x_his):
        ...
        return xs_rec, elbo_s, mlbo_s,qz_s
...
class TNet(nn.Module):
    ...
    def forward(self, x_his):
        ...
        return xt_rec, elbo_t, mlbo_t,qz_t


class LaST(nn.Module):
    def __init__(self, input_len, output_len, input_dim, out_dim, num_class,var_num=1, latent_dim=64, dropout=0.1,
                 device="cuda:0"):
        ...
        self.classification = nn.Sequential(nn.Linear(2*input_len * latent_dim,512),
                                            nn.ReLU(),
                                            nn.Linear(512,128),
                                            nn.ReLU(),
    ...
    def forward(self, x, x_mark=None):
        ...
        elbo, mlbo, mubo,xs_rec,xt_rec,qz_s,qz_t = self.LaSTLayer(x_his)

        # if x_t.shape[1] == 0: #no forecasting
        #     return None, elbo, mlbo, mubo,xs_rec,xt_rec,qz_s,qz_t
        # x_pred = x_s + x_t
        # x_pred = x_pred.squeeze(-1) if self.v_num > 1 else x_pred
        qz_s = qz_s.reshape(qz_s.shape[0], -1)
        qz_t = qz_s.reshape(qz_t.shape[0], -1)
        x_pred = self.classification(torch.concat([qz_s,qz_t],1))
        return x_pred, elbo, mlbo, mubo,xs_rec,xt_rec,qz_s,qz_t
